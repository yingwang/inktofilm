...
VidSpec includes Codex CLI planning and judging plus fal MiniMax H3 video generation. Each role can
...
schema; VidSpec validates IDs, shot durations, aspect ratio, assertions, and duplicate shots again.
...
`{"video":"/absolute/path/to/result.mp4"}` and VidSpec will copy that local file into the production
...
`--judge-command` uses the same request and response contract as `vidspec run --semantic-command`.
...
- VidSpec records provider provenance and local output paths, never credential values.
